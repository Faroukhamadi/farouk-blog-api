...
func (cq *ContentQuery) QueryPost() *PostQuery {
...
func (cq *ContentQuery) WithPost(opts ...func(*PostQuery)) *ContentQuery {
	query := &PostQuery{config: cq.config}
...
func (cq *ContentQuery) GroupBy(field string, fields ...string) *ContentGroupBy {
	grbuild := &ContentGroupBy{config: cq.config}
...
func (cq *ContentQuery) Select(fields ...string) *ContentSelect {
	...
	selbuild := &ContentSelect{ContentQuery: cq}
	selbuild.label = content.Label
...
func (cq *ContentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Content, error) {
	...
	if query := cq.withPost; query != nil {
		if err := cq.loadPost(ctx, query, nodes, nil,
			func(n *Content, e *Post) { n.Edges.Post = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ContentQuery) loadPost(ctx context.Context, query *PostQuery, nodes []*Content, init func(*Content), assign func(*Content, *Post)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Content)
	for i := range nodes {
		if nodes[i].post_content == nil {
			continue
		}
		fk := *nodes[i].post_content
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "post_content" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
