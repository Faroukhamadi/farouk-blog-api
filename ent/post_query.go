...
func (pq *PostQuery) QueryContent() *ContentQuery {
	query := &ContentQuery{config: pq.config}
...
func (pq *PostQuery) Clone() *PostQuery {
	...
	return &PostQuery{
		...
		withContent: pq.withContent.Clone(),
...
func (pq *PostQuery) WithContent(opts ...func(*ContentQuery)) *PostQuery {
	query := &ContentQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withContent = query
	return pq
}
...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if query := pq.withContent; query != nil {
		if err := pq.loadContent(ctx, query, nodes,
			func(n *Post) { n.Edges.Content = []*Content{} },
			func(n *Post, e *Content) { n.Edges.Content = append(n.Edges.Content, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PostQuery) loadContent(ctx context.Context, query *ContentQuery, nodes []*Post, init func(*Post), assign func(*Post, *Content)) error {
	...
	query.Where(predicate.Content(func(s *sql.Selector) {
		s.Where(sql.InValues(post.ContentColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.post_content
		if fk == nil {
			return fmt.Errorf(`foreign-key "post_content" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "post_content" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
